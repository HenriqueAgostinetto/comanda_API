...
const UserProfile = ({ anchorEl, onClose }) => {
  ...
  return (                 // henrique agostinetto piva
    <Menu
      anchorEl={anchorEl}
      open={open}
      onClose={onClose}
      ...
      <Box sx={{ p: 2, minWidth: 280 }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', mb: 2 }}>
          <img 
            src="/minha_foto.jpg" 
            alt="foto de perfil" 
            style={{ width: '80px', height: '80px', borderRadius: '50%', objectFit: 'cover', border: '2px solid #f59e0b' }} 
          />
        </Box>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            ...
            {user.matricula && (
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <Typography variant="body2" color="text.secondary">matricula: {user.matricula}</Typography>
              </Box>
            )}
          </Box>
        )}
